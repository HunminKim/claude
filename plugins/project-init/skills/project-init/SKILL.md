...
4. **AskUserQuestion 툴**로 확인한다:
   - 질문: "위 선별 결과로 진행할까요?"
   - 옵션: `["이대로 진행합니다 (Recommended)", "유지/제거 목록을 조정하겠습니다"]`
...
└── llm-agent.md                ← LLM 에이전트 구현 서브에이전트 (프롬프트/툴/오케스트레이션/eval)
.claude/
...
6. `.claude/agents/` 6종 — `verifier.md`, `infra.md`, `backend.md`, `frontend.md`, `deeplearning.md`, `llm-agent.md` — 모든 hook 스크립트 생성 후 생성
...
**포함할 것** (코드를 읽어도 알 수 없는 **이 프로젝트 고유** 정보만):
- 프로젝트 특화 명령어 (빌드/테스트/린트)
- 비표준 코드 스타일 (예: 한국어 주석 규칙)
- 이 프로젝트만의 비직관적 제약·함정 (예: 특정 라이브러리 버그, GPU 제약, 경로 분리 규칙)
- 알려진 버그 / 환경 제약
- 아키텍처 결정 및 비직관적인 주의사항
- 그리고 "규칙 분산 구조" 표 (어떤 규칙이 어느 레이어에 있는지 가리키는 포인터 — 템플릿 참조)

**제외할 것** (CLAUDE.md 는 매 세션 always-load 라 토큰을 매 턴 소비 — 절차는 조건부 로드 레이어로):
- **제네릭 절차·워크플로우 규칙** — 서브에이전트 위임, plan-gate 사용법, verifier 흐름, /compact 타이밍, 커밋 형식, Think Before Coding, 버그 보고 절차 등 **모든 프로젝트 공통 규칙**. 이들은 `.claude/memory/workflow.md`(과정 규칙)·`.claude/rules/code-style.md`(코드 규칙)에만 둔다. CLAUDE.md 에 중복 기재 금지 — 양쪽에 적으면 drift·토큰 낭비. (이게 198줄 비대화의 근본 원인이었다)
- `.claude/rules/code-style.md`·`.claude/memory/workflow.md`에 이미 있는 내용 — 중복 금지
...
4. **AskUserQuestion 툴**로 덮어쓰기 허가를 받는다:
   - 질문: "CLAUDE.md를 새 내용으로 덮어쓸까요?"
   - 옵션: `["덮어쓰기 (Recommended)", "건드리지 않고 넘어가기"]`
...
> 블록으로 이번 작업이 건드릴 파일 패턴을 선언하면, 스코프 밖 편집이 기본적으로
> 환기된다(shadow). `/plan-gate-scope-enforce` 로 올리면 차단까지 한다. `*`=한 경로
> 단계, `**`=하위 전체. enforce 는 게이트가 닫히면 shadow 로 자동 복귀하며, 자동으로
> enforce 를 켜지는 않는다(파괴적 강제는 명시 opt-in). git 저장소에서만 Bash 변경
> 롤백(layer-2)이 동작한다.
...
### .claude/agents/llm-agent.md 템플릿

`assets/templates/agents/llm-agent.md` 파일을 읽어 사용한다.

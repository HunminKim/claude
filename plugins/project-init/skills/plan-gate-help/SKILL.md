...
## 차단 코드 → 대응 커맨드 (메시지 헤더의 `[코드]` 로 원인 판별)

| 코드 | 원인 | 대응 |
|------|------|------|
| `PG-TRIGGER` | 복잡도 임계 — 계획 승인 필요 | todo.md 작성 → `/approve-plan` (아래 "plan-gate가 차단했을 때") |
| `PG-D1` | verifier ❌ 미해결 편집 잠금 | `/retry` · `/skip` · `/done` · `/rollback` (아래 "@verifier 검증 후") |
| `PG-THRASH` | 같은 파일 반복 편집(수렴 안 됨) | 접근 재검토 · `/replan` (아래 "반복 편집 차단") |
| `PG-SCOPE-L1` | 스코프 밖 편집 거부(enforce) | `/subplan <패턴>` 또는 매니페스트 수정 |
| `PG-DNT` | do-not-touch 위반 (subplan 으로도 확장 불가) | 매니페스트 재검토 후 `/replan` |
| `PG-SCOPE-SHADOW` / `PG-SCOPE-L2` | 위반 감지(기록/스윕) | 필요한 변경이면 scope 에 추가 |
| `FL-LOOP` | 연속 Bash 실패 — **failure-loop 가드, plan-gate 아님** | 커맨드 없음 — 멈추고 원인 분석·보고 |

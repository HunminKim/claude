...
- [x] `/prompt-log-status` 슬래시 커맨드 — 현재 프로젝트 동의 여부, 글로벌 통계 ✅
...
- [x] 사용자 정의 sanitize 규칙 `~/.claude/prompt-log/sanitize_rules.yaml` ✅
- [ ] 환경변수 강제 비활성 (`PROMPT_LOG=0` 또는 `PL_DISABLED=1`)
- [x] 파일 권한 0600 강제 (`chmod 0600` 자동 적용) ✅
...
- [x] `fcntl.flock` 으로 jsonl append 시 동시 쓰기 방지 ✅
- [x] active state 파일 lock (여러 PreToolUse 동시 실행 시) ✅

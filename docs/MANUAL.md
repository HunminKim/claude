...
9. [보조 스킬 (monitor / harness-update / harness-uninstall / plan-gate-help)](#9-보조-스킬)
...
3. **4계층 규칙 구조** 생성 (아래)
4. **6개 서브에이전트** (verifier + 도메인 5) + 6개 로컬 훅 + `.githooks/` 생성
...
상태의 루트는 **작업 트리 기준**이다 — linked git worktree 에서 작업하면 워크트리 자체의
`.claude/state/` 에 분리 저장되어 원본 체크아웃의 병렬 세션과 카운터·전이가 섞이지 않는다
(활성 플래그 `plan_gate_enabled` 는 워크트리에 없으면 원본 것을 따른다 — 설정 공유·상태 분리, v2.20.0).
...
슬래시 명령으로 호출한다. 일부는 평문 토큰(예: `done`, `approve`)으로도 인식된다(`plan_approval.py` fallback — 프롬프트 전체가 토큰과 정확히 일치할 때만, 오발 방지).

> 전이 명령(approve/done/…)은 **사용자 전용 결정**이다. Claude 가 이를 Bash 로 직접 호출하면(`plan_gate_cli.py approve` 등) 훅이 사용자 확인창(ask)으로 승격한다 — [5.1](#51-bash-안전-가드-상세-dangerous_bash_check) 참조.
...
- **판정은 지금 편집하려는 파일 기준**: 그 파일 자체가 5회를 넘겼을 때만 막는다. 다른 파일이 thrash 여도 무관한 파일 편집은 연좌 차단하지 않는다 (v2.20.0).
...
project-init 의 18개 훅이 이벤트별로 묶여 있다(`detect_failure_loop` 와 `plan_gate_bash` 는 PostToolUse·PostToolUseFailure 두 이벤트에 등록돼 표에선 2회 나타난다 — 등록 기준 20개). **출력 채널**은 의도와 1:1:
`차단`=exit2+stderr, `환기`=additionalContext JSON(Claude 가 봄), `ask`=permissionDecision:ask JSON(사용자 확인창으로 승격), `사용자전용`=stderr.

| 이벤트 | 매처 | 훅 | 역할 | 채널 |
|--------|------|----|------|------|
| **PreToolUse** | Bash | `dangerous_bash_check` | `rm -rf /`·비밀파일·인라인 토큰 등 차단 + 워크스페이스 밖 파괴 명령·plan-gate 자가전이 ask 승격 (아래 5.1) | 차단/ask |
...
| **PostToolUseFailure** | Bash | `detect_failure_loop` | 실패 이벤트 포착 (멀티스키마 분류) | 환기/차단 |
| | Bash | `plan_gate_bash` | 실패한 Bash 의 스코프 밖 생성물도 스윕 (green-reset 은 안 함) | 환기 |
| **UserPromptSubmit** | * | `detect_bug_report` | 버그 키워드 → "고치기 전 생각" 체크리스트 | 환기 |
| | * | `detect_user_correction` | 한국어 교정 신호 → lessons.md 환기 | 환기 |
| | * | `plan_approval` | 평문 토큰(done/approve 등, 정확일치) → CLI fallback | — |
| | * | `detect_task_boundary` | 타임아웃(60분) 자동 done / replan vs done 선택 | 환기 |
...
### 5.1 Bash 안전 가드 상세 (`dangerous_bash_check`)

모든 Bash 명령이 실행 전에 이 훅을 거친다. 판정은 강한 순서대로 4단계이며, 각 단계의 채널이 다르다:

| 단계 | 감지 대상 | 채널 |
|------|----------|------|
| ① 하드 블록 | `rm -rf /`·`rm -rf ~`(래퍼·쿼팅·긴옵션·`$HOME` 변형 포함), `find / -delete`, fork bomb, `dd of=/dev/sdX`, `mkfs`, 프로젝트 루트(`CLAUDE_PROJECT_DIR`) 통째 삭제, 핵심 운영 파일(Dockerfile·.env 등) 삭제 | 차단 (exit 2) |
| ② 비밀 노출 | 비밀 파일(.env·키·자격증명)의 읽기/소싱/리다이렉트/복사/아카이브/업로드 — fail-closed 광역 탐지 | 차단 (exit 2) |
| ③ 인라인 시크릿 | 명령 문자열 안의 API 키·PAT·AWS 키·URL 내장 인증정보 | 차단 (exit 2) |
| ④ **workspace-guard** | 파괴적 명령(`rm`/`mv`/`shred`/`rmdir`/`unlink`/`find -delete`/`-exec rm`/`xargs rm`/`>` truncate)의 타겟이 **워크스페이스 밖**(상위·형제 디렉토리)으로 리졸브 | **ask** (사용자 확인창) |

**workspace-guard 판정 규칙** (④):

- 타겟 경로는 훅 payload 의 `cwd` 기준 `realpath` 로 리졸브 — 절대경로, `../` 상대경로, `~` 홈, glob(`../*`), 따옴표 타겟, `sudo` 등 래퍼 경유, `sh -c '…'` 인용 중첩 모두 동일하게 판정된다.
- **변수·명령치환 타겟은 fail-closed**: `$UNDEFINED/x`·`$(pwd)/x` 처럼 정적으로 해석 불가능하면 ask 로 승격한다 (훅 환경변수로 해석되는 `$VAR` 는 해석 결과로 판정).
- **시스템 임시 트리 예외**: `/tmp`·`/var/tmp`·`/dev/shm` 하위는 스크래치 용도로 통과. 단 워크스페이스 자체가 그 트리 안이면 해당 트리 예외가 꺼진다 (같은 트리의 형제 프로젝트 삭제 방지).
- 차단이 아니라 **ask 인 이유**: 사용자가 정당하게 요청한 워크스페이스 밖 정리까지 죽이지 않기 위해서다. 확인창에서 한 번 허용하면 실행되고, Claude 의 자율 판단이면 거부하면 된다.
- `CLAUDE_PROJECT_DIR` 미설정 환경에선 판정 불가로 조용히 통과한다 (fail-open — 기존 프로젝트 루트 삭제 가드와 동일 정책).

이 훅은 추가로 **plan-gate 자가전이 승격**도 담당한다: Claude 가 `plan_gate_cli.py approve|done|…` 를 Bash 로 직접 호출하면(슬래시 커맨드 우회) ask 로 승격해 사용자가 확인하게 한다 — `status`·`subplan` 같은 조회·의도적 Claude 경로는 무간섭.

---

## 6. 서브에이전트

project-init 이 `.claude/agents/` 에 생성하는 6개 (verifier + 도메인 5). `@이름` 으로 호출한다.
...
- `/del_prompt_log` — 대화형 정리: 수집된 로그를 `./prompt_log/` 로 이동(원본 삭제) 후 동의 유지/철회를 재확인
- 완전 제거(비대화형): `plugins/prompt-log/uninstall.sh` (글로벌 데이터 + 모든 마커 삭제). 모든 코드는 `[prompt-log]` 마커로 grep 가능.
- ⚠️ V1 은 동의 철회 후 자동 삭제가 없다(`/del_prompt_log` 또는 수동 rm 필요, V2 예정).
...
- 업데이트 중 plan-gate 자동 비활성화 → 끝나면 복원 (중단돼도 다음 실행이 자가복구)
- placeholder-aware diff (`{{...}}` 변수 무시, 재주입 방지)
- **절대 안 건드림**: `CLAUDE.md`, `.claude/memory/lessons.md`, `docs/`, `tasks/`, `.claude/state/`

### `/harness-uninstall` — 하네스 제거 (문서 보존)

이 프로젝트에서 하네스(훅·에이전트·plan-gate 런타임·git hooks)만 걷어낸다.
`CLAUDE.md`·`docs/`·`tasks/` 등 문서와 사용자 추가분은 보존하며, 삭제 목록을 먼저 보여주고 확인 후 진행한다.

### `/plan-gate-help` — plan-gate 상황별 안내

"지금 뭘 입력해야 하나"를 상황(승인 대기·검증 ❌·스코프 차단 등)별로 안내하는 조회용 스킬.
명령 레퍼런스를 CLAUDE.md 에 상주시키는 대신 필요할 때만 로드하는 progressive disclosure 설계다.
...
**Q. 워크스페이스 밖 파일을 지우려는데 확인창(ask)이 뜬다.**
- 의도된 보호(workspace-guard, [5.1](#51-bash-안전-가드-상세-dangerous_bash_check)). 상위·형제 디렉토리를 향한 파괴적 명령은 사용자 확인 후에만 실행된다. 내가 시킨 작업이면 **허용**을 누르면 되고, Claude 가 스스로 벌인 일이면 거부한다. `$VAR`·`$(…)` 타겟도 해석 불가라 확인창이 뜬다 — 경로를 리터럴로 풀어 쓰면 워크스페이스 안은 통과된다.
...
| `.claude/state/plan_gate_audit.log` | 게이트 전이·스코프 위반·subplan 확장·롤백 감사 로그 |

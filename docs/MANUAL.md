...
> (작성 시점 버전: marketplace `hunminkim`, project-init **v2.6.0**, harness-check **v1.0.1**, prompt-log **v1.1.1**)
>
...
| 플러그인 | 버전 | 한 줄 요약 |
|---------|------|----------|
| **project-init** | v2.6.0 | 본체. 프로젝트 스캐폴딩 + plan-gate + verifier + 21개 훅 + 18개 명령 |
...
│   ├── hooks/                6개 로컬 훅
│   ├── agents/               verifier + backend/frontend/infra/deeplearning
...
| 명령 | 모드 | 효과 |
|------|------|------|
| `/plan-gate-scope-off` | off (기본) | 매니페스트 기록만, 차단·롤백 없음 |
| `/plan-gate-scope-shadow` | shadow | 위반 **감지·기록만** (롤아웃 관찰용) |
...
모드는 `.claude/plan_gate_scope` 파일 내용(off/shadow/enforce)으로 표현된다. 부재 시 off.
...
| **deeplearning** | sonnet | 모델·학습루프·데이터 전처리·평가 | 배포 인프라(GPU 프로비저닝 등) |
...
| `.claude/plan_gate_scope` | 내용 = `off`/`shadow`/`enforce` (부재 = off) |

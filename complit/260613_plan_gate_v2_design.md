...
- **(step 5 후속) `subplan` CLI [R5 충족 — R1 출하 완료, 이제 GO 가능]**: R1 스윕 롤백이 동작하므로 subplan 출하 선결조건 충족. 단 step 5 본체에서 **의도적 분리**(Simplicity-First — 핵심 강제부터 안정화). subplan = 승인 scope 안에서 더 좁은 하위 스코프를 Claude 가 선언(위임 서브에이전트용). 구현 시: gate `expansions` 필드 + `subplan` CLI(**Claude 호출 가능 — `_ACTION_TOKENS` 에 넣지 말 것, `disable-model-invocation` 부여 금지**).

...
### [문제 #6] verifier 의 `.verifier_result.json` Write 거부 → 게이트 상태머신 deadlock

**발견 시점**: Phase 6 개정 cycle 의 1차 verifier 호출 (2026-05-15)

**증상**: verifier 가 검증을 완료하고 ❌ 판정을 텍스트로 보고했으나, `docs/.verifier_result.json` Write 가 harness 에 의해 거부됨 ("Write permission ... has been denied"). 결과로:
- `update_docs.py` PostToolUse 훅이 발동하지 않음 → 게이트 `verifier_status` 가 `null` 유지
- 게이트 state 는 `approved` 그대로
- `/retry`, `/skip` 모두 `state==verified && verifier_status==❌` 를 요구 → **deadlock**
- 출구는 `/done` (거짓 ✅ 처리) 또는 `/replan` (게이트 park) 만 남음
- 실제로 `plan_gate.json` 의 `file_edit_counts` 에 `.verifier_result.json: 1` 이 기록됨 — PreToolUse 훅은 Write 를 감지했으나 실제 파일은 생성 안 됨

**추정 원인**: verifier 가 async/background 로 실행되면 인터랙티브 권한 승인을 받을 수 없음. Claude Code 의 권한 시스템이 `docs/.verifier_result.json` Write 를 자동 허용하지 않으면 거부됨.

**영향 파일** (수정 후보)
- 권한 시스템: settings template 의 `permissions.allow` 에 `Write(docs/.verifier_result.json)` 자동 허용 추가
- 또는 `plan_gate_cli.py`: verifier 가 텍스트 verdict 만 남기고 JSON Write 가 실패한 경우를 감지하는 안전망 (예: `cmd_retry`/`cmd_skip` 이 `state==approved && 최근 verifier 에이전트 종료 흔적 있음` 인 경우에도 동작하도록, 또는 verifier 결과 수동 등록용 별도 명령)
- 또는 verifier 호출 패턴: 메인이 verifier 호출 직후 `.verifier_result.json` 존재 여부를 확인하고, 부재 시 verifier 에게 SendMessage 로 재시도 요청 (verifier.md 의 "이 파일은 verifier만 생성" 규칙 유지)

**수정 방향 (최소 침습)**: settings template (`plugins/.../settings.json` 또는 hook 설치 스크립트) 에 다음 permission 자동 허용 추가:
```json
{
  "permissions": {
    "allow": [
      "Write(./docs/.verifier_result.json)"
    ]
  }
}
```
이러면 verifier 가 async 로 실행돼도 해당 파일 Write 는 자동 통과.

**수정 이유**: verifier 의 단일 필수 출력 경로(`docs/.verifier_result.json`)가 권한 시스템에 막히면 게이트 전체가 deadlock 되는 구조. 권한 허용을 plugin 설치 시점에 보장하거나, harness 가 이 deadlock 을 감지·복구할 수 있어야 한다.

**우선순위**: 중간 (현재 우회 가능 — `/replan` 으로 park; 그러나 verifier 절차 정상 동작 보장의 핵심)
